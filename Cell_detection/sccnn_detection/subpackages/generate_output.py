...
def make_sub_dirs(opts, sub_dir_name):
    if not os.path.isdir(os.path.join(opts.results_dir, 'network_output', sub_dir_name)):
        os.makedirs(os.path.join(opts.results_dir, 'network_output', sub_dir_name))
    ...
    if not os.path.isdir(os.path.join(opts.results_dir, 'detected_points', sub_dir_name)):
        os.makedirs(os.path.join(opts.results_dir, 'detected_points', sub_dir_name))
...
def pre_process_images(opts, sub_dir_name, eng=None):
    if eng is None:
        ...
        eng.addpath(eng.genpath(os.path.join(curr_file_dir, '..', '..', 'sccnn_detection/')), nargout=0)
...
def post_process_images(opts, sub_dir_name, eng=None):
    ...
    maxclique = {'distance': opts.maxclique_distance,
                 'threshold': opts.maxclique_threshold}

    make_sub_dirs(opts, sub_dir_name)
    eng.save_detection_output_p(opts.results_dir, sub_dir_name, os.path.join(opts.data_dir, sub_dir_name), maxclique,
                                nargout=0)


def generate_network_output(opts, sub_dir_name, network, sess, logits, eng=None):
    if eng is None:
        eng = matlab.engine.start_matlab()
        curr_file_path = os.path.realpath(__file__)
        curr_file_dir = os.path.dirname(curr_file_path)
    ...
    for i in range(len(files_tissue)):
        ...
        if not os.path.isfile(os.path.join(opts.results_dir, 'network_output', sub_dir_name, file_name + '.mat')):
            print(file_name, flush=True)
            image_path_full = os.path.join(opts.data_dir, sub_dir_name, file_name + '.jpg')
            if opts.pre_process:
                eng.load(os.path.join(opts.preprocessed_dir, 'pre_processed', sub_dir_name, file_name + '.mat'),
                         nargout=0)
                feat = eng.workspace['feat']
                feat = np.array(feat._data).reshape(feat.size, order='F')
            else:
            ...
            h5.h5write(filename=os.path.join(opts.results_dir, 'network_output', sub_dir_name, h5_file_name),
                       data_to_save=output,
                       name_data=name_data)
            to_eval = "output = h5load('" + \
                      os.path.join(opts.results_dir, 'network_output', sub_dir_name, h5_file_name) + \
                      "', '" + name_data + "');"
            eng.eval(to_eval, nargout=0)
            eng.save(os.path.join(
                opts.results_dir, 'network_output', sub_dir_name, file_name + '.mat'), 'output', '-v7.3',
                nargout=0)
            eng.delete(os.path.join(opts.results_dir, 'network_output', sub_dir_name, h5_file_name), nargout=0)
