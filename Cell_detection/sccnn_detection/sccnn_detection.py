...
class SCCNN:
    ...
    def inference(self, images=None):
        if images is None:
            images=self.images
        self.logits = inference.inference(network=self, images=images)
        return self.logits
    ...
    def loss_function(self, logits=None, labels=None):
        ...
        if labels is None:
            labels = self.labels
        self.loss = loss_function.loss_function(network=self, logits=logits, labels=labels)
        return self.loss

    def train(self, loss=None):
        if loss is None:
            loss = self.loss
        self.train_op = train_op.train(network=self, loss=loss)
