...
cp /mnt/scratch/users/molecpath/sraza/scripts/MyCodes/TracerX/20171019-SCCNNClassifier/parameters-classification.txt "${1}"
FULLDIRNAMEESC=$(echo $FULLDIRNAME | sed 's_/_\\/_g') 
for s in $SAMPLES; do	
  echo "Create bsub for ${s}"
  BNAME=`basename ${s}`
  cp /mnt/scratch/users/molecpath/sraza/scripts/MyCodes/TracerX/20171019-SCCNNClassifier/header_cell_classification "${FULLDIRNAME}/bsubclassification/${BNAME}_extract.bsubclassification"  
